...
public class ServiceOfferedController {
    ...
    @Operation(
            summary = "Add service",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Successfully added service", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ServiceOfferedDTO.class))),
    ...
    public ResponseEntity<?> addService(@RequestBody @Valid ServiceOfferedDTO serviceOfferedDTO) {
        ...
        return ResponseEntity.ok(
                messageSource.getMessage("service.add.success", null, LocaleContextHolder.getLocale()));
    }
